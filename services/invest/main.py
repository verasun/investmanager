#!/usr/bin/env python
"""Invest Service - Main entry point.
...
class LLMClient:
    ...
    async def parse_intent(self, message: str) -> dict:
        """Call intent parsing through Gateway."""
        client = await self._get_client()

        try:
            response = await client.post(
                f"{self.gateway_url}/llm/intent",
...
class PersonalizationManager:
    """Manages user personalization for investment advice."""
    ...
    async def get_personalized_prompt(self, user_id: str) -> Optional[str]:
        """Get personalized system prompt for investment mode."""
        self._load_modules()
...
class InvestService:
    ...
    async def handle_message(self, context: MessageContext) -> HandleResponse:
        ...
        logger.info(f"[{trace_id}] InvestService: {text[:50]}... from {user_id}")
        ...
        system_prompt = await self.personalization.get_personalized_prompt(user_id)
        ...
        try:
            ...
            logger.error(f"[{trace_id}] InvestService error: {e}")
            return HandleResponse(
                success=False,
...
# Global instances
_invest_service: Optional[InvestService] = None


def get_invest_service() -> InvestService:
    """Get or create Invest service instance."""
    global _invest_service
    if _invest_service is None:
        _invest_service = InvestService(GATEWAY_URL, SERVICE_API_KEY)
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Starting Invest Service...")
    ...
    from services.capability_protocol import get_invest_capability

    capability = get_invest_capability()
    ...
    # Cleanup
    logger.info("Shutting down Invest Service...")
    service = get_invest_service()
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="InvestManager Invest Service",
...
def register_routes(app: FastAPI):
    ...
    async def root():
        """Root endpoint."""
        return {
            "name": "InvestManager Invest Service",
            "version": "1.0.0",
            "gateway_url": GATEWAY_URL,
        }
    ...
    async def handle_message(context: MessageContext):
        """Handle a message in investment mode.
    ...
    async def analyze_stock(request: AnalyzeRequest):
        ...
        return {
            ...
            "message": "Stock analysis will be implemented in a future version",
        }
    ...
    async def backtest_strategy(request: BacktestRequest):
        ...
        return {
            ...
            "status": "not_implemented",
            "message": "Backtesting will be implemented in a future version",
        }
...
def run_invest_service():
    """Run the Invest service."""
    import uvicorn

    parser = argparse.ArgumentParser(description="InvestManager Invest Service")
    ...
    uvicorn.run(
        "services.invest.main:create_app",
        host=args.host,
        port=args.port,
        factory=True,
        reload=args.reload,
